...
int main (int argc, char *argv[]) {
  ...
  while ( a < argc && argv[a][0] == '-' ) {
    ...
    } else if ( strcmp(argv[a], "-incfile") == 0 ) {
      if ( (a+1) < argc ) {
	opari_incfile_name = new char[strlen(argv[++a])+1];
	sprintf(opari_incfile_name, "%s", argv[a]);
      } else {
        cerr << "ERROR: missing value for option -incfile\n";
        errFlag = true;
      }
  ...
  if ( errFlag ) {
    cerr << "usage: " << argv[0] << " [-f77|-f90|-c|-c++] [-nosrc]\n"
         ...
         << "       [-rcfile <file>] [-table <tabfile>] [-incfile <incfile>] <infile> [<outfile>]\n";
  ...
  if ( tabfile && (a == argc) ) {
    ...
    generateTableFile(rcfile, tabfile);
  } else {
    // generate opari include file name
    if ( !opari_incfile_name) {
      opari_incfile_name = new char[strlen(infile)+12];
      sprintf(opari_incfile_name, "%s.opari.inc", infile);
    }
    ...
    if ( lang & L_FORTRAN ) {
      ...
      process_fortran(is, infile, opari_incfile_name, os);
    } else {
      define_POMP(os);
      os << "#include \"" << opari_incfile_name << "\"" << "\n";
      if ( keepSrcInfo ) os << "#line 1 \"" << infile << "\"" << "\n";
      process_c_or_cxx(is, infile, opari_incfile_name, os);
    }
    finalize_handler(opari_incfile_name, tabfile);
    delete opari_incfile_name;
  }
