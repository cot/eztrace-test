...
# $(NM) $ALL_OBJS_AND_LIBS | $(GREP) -i POMP2_Init_regions | $(GREP) " [TD] "
...
/_[0-9][0-9]*_[0-9][0-9]*_?_?$/ {  
  for (i = 1; i <= NF; i++)
  {
    ...
    {
      separator = "POMP2_Init_regions";
    }
    else if (index($i,"pomp2_init_regions") != 0)
    {
      separator = "pomp2_init_regions";
    }
    else if (index($i,"POMP2_INIT_REGIONS") != 0)
    ...
    # $i looks like "POMP2_Init_regions_uniqueId_nRegions" or
    # like "pomp2_init_regions_uniqueId_nRegions" or
    # like "POMP2_INIT_REGIONS_uniqueId_nRegions"
