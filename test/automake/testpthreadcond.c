#define nproc 1
#define nameprog "test/pthread/cond"
...
void init_tabevent() {
	...
	nbdiffevent=0;
		for (j=0; j<nproc; j++) {
			...
			listevent[j][0].nbeventexpected=0;
		}
		nbdiffevent++;
		for (j=0; j<nproc; j++) {
		...
		for (j=0; j<nproc; j++) {
			...
			listevent[j][9].nbeventexpected=1;
		}
		nbdiffevent++;
		for (j=0; j<nproc; j++) {
			listevent[j][10].code="20034";
			listevent[j][10].nbeventexpected=1;
		}
		nbdiffevent++;
		for (j=0; j<nproc; j++) {
			listevent[j][11].code="20035";
			listevent[j][11].nbeventexpected=1;
		}
		nbdiffevent++;
		for (j=0; j<nproc; j++) {
			listevent[j][12].code="20040";
			listevent[j][12].nbeventexpected=1;
		}
		nbdiffevent++;
		for (j=0; j<nproc; j++) {
			listevent[j][13].code="20041";
			listevent[j][13].nbeventexpected=1;
		}
		nbdiffevent++;
		for (j=0; j<nproc; j++) {
