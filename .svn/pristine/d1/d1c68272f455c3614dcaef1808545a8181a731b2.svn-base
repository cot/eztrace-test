...
namespace {
  ...
  void h_task(OMPragma* p, ostream& os) {
    int n = nextID();
    OMPRegion* r = REnter(p, n);
    print_pragma(p, os);
    generate_call("begin", "task", n, os);
  }

  void h_endtask(OMPragma* p, ostream& os) {
    int n = RExit(p);
    generate_call("end", "task", n, os);
    print_pragma(p, os);
    if ( keepSrcInfo ) reset_src_info(p, os);
  }

  void h_taskwait(OMPragma* p, ostream& os) {
    int n = nextID();
    OMPRegion* r = new OMPRegion(p->name, p->filename, n,
                                 p->lineno, p->lineno+p->lines.size()-1);
    regions.push_back(r);
    generate_call("enter", "taskwait", n, os);
    print_pragma(p, os);
    generate_call("exit", "taskwait", n, os);
    if ( keepSrcInfo ) reset_src_info(p, os);
  }
