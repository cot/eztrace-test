...
int main(int argc, char **argv) {
  ...
  if (test==0) {
     strcat(ezt_unused_path, ":");
     }

  char ezt_all[SIZE_STRING * 2] = "";
  strcat(ezt_all, ezt_trace);
  strcat(ezt_all, ezt_unused);
  if(ezt_all != NULL) setenv("EZTRACE_ALL",ezt_all,1);
  tmp = getenv("EZTRACE_ALL");
  printf(" EZTRACE_ALL = %s \n",tmp);

  char ezt_all_path[SIZE_STRING * 2] = "";
  strcat(ezt_all_path, ezt_library_path);
  strcat(ezt_all_path, ":");
  strcat(ezt_all_path, ezt_unused_path);
  ...
  if (strcmp(ezt_all_path, "")) {
	  ...
	  while(dir != NULL) {
		  if (strcmp(dir, "")) {
			  if(!strcmp(dir,"/usr/local/lib" ))  nb_dirs++;
		  }
		  dir = strtok(NULL, ":");
	  }
  }
  ...
  if(nb_dirs != 0) {
	  ...
	  tmp = NULL;
	  // on remplit d'abord avec les modules d'EZTRACE_LIBRARY_PATH
	  tmp = getenv("EZTRACE_ALL_PATH");
	  strcpy(ezt_all_path, "");
	  if(tmp != NULL) {
  ...
  if (test) {
	  ...
	  strcat(files, "/home/rue/Bureau/EZTRACE/eztrace/unused/src/core/.libs/libeztrace-autostart.so");
  }else{
