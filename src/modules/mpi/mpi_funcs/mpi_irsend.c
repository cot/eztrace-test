...
static int MPI_Irsend_core (CONST void* buf, int count, MPI_Datatype datatype, int dest, int tag,
			    MPI_Comm comm, MPI_Request *req)
{
  return libMPI_Irsend(buf, count, datatype, dest, tag, comm, req);
}

static void MPI_Irsend_epilog (CONST void* buf __attribute__((unused)),
			       int count,
			       ...
			       MPI_Fint *req)
{
  ...
  EZTRACE_EVENT5 (FUT_MPI_IRSEND, count*size, dest, tag, req, comm);
}


int MPI_Irsend (CONST void* buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *req)
{
  FUNCTION_ENTRY;
...
void mpif_irsend_(void *buf, int *count, MPI_Fint *d, int *dest,
