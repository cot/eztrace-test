...
static int MPI_Ibsend_core (CONST void* buf, int count, MPI_Datatype datatype, int dest, int tag,
			    MPI_Comm comm, MPI_Request *req)
{
  return libMPI_Ibsend(buf, count, datatype, dest, tag, comm, req);
}

static void MPI_Ibsend_epilog (CONST void* buf __attribute__((unused)),
			       int count,
			       ...
			       MPI_Fint *req)
{
  ...
  EZTRACE_EVENT5 (FUT_MPI_IBSEND, count*size, dest, tag, req, comm);
}

int MPI_Ibsend (CONST void* buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *req)
{
  FUNCTION_ENTRY;
